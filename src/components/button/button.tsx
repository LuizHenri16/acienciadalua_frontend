...
export function Button({ name, loadingName, onClick, variant = 'primary', loading = false }: ButtonProps) {
    return (
        <button className={`w-full py-4 mt-5 cursor-pointer rounded-xl shadow-md font-bold transition-colors ${variant === 'primary' ? 'bg-[#68B999] hover:opacity-90 text-white' : 'bg-[#3D6B5C] hover:opacity-90 text-[#E5E5E3]'}`}
            onClick={onClick}
            disabled={loading}>
